...
-- 요앙사 테이블에 레코드(이름, 아이디, 암호화된 비밀번호, 전화번호, 가입경로) 를 생성하는 행위
-- 필요한 추가 데이터 : 가입경로 (화면에서 가져옴), 암호화된 비밀번호 (서버가 생성)

-- 레코드 삽입시 아이디 중복 검증
SELECT * FROM nurse WHERE id = 'qwer1234';
-- 레코드 삽입시 전화번호 중복 검증
SELECT * FROM nurse WHERE tel_numbewr = '01012345678';
-- 절차상에 인증된 전화번호만 삽입 가능
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND '0123';

INSERT INTO nurse VALUES ('qwer1234', 'qwerasdfzxcv', '홍길동', '01012345678', 'HOME');
...
-- 전화번호 인증 
-- 최종사용자 (전화번호)
-- 서버측에서 인증번호를 생성하여 사용자가 입력한 전화번호에 전송 후 전화번호 인증 테이블에 레코드를 추가하는 작업 (전화번호, 인증번호)를 추가
-- 필요한 추가 데이터 : 인증번호 (서버가 생성)

-- 레코드 삽입시 이미 사용중인 전화번호 인지 검증
SELECT * FROM nurse WHERE tel_number = '01012345678';
-- 레코드 삽입시 이미 인증 절차를 거지고 있는 전화번호인지 확인
...
SELECT *FROM tel_auth WHERE tel_number = '01012345678' AND tel_auth = '0123';
...
-- 고객 테이블에서 페이지 수에 해당하는 위치부터 10개의 고객 수를 내림차순 반환
...
-- 고객 테이블에서 이름이 포함되어 있는 레코드들을 페이지 수에 해당하는 위치부터 10개의 고객 수를 내림차순 반환
...
-- 고객 삭제
-- 최종 사용자 (고객 정보)
-- 고객 테이블에서 지정한 고객(고객 번호)의 레코드를 삭제하는 작업
...
-- 고객 테이블에 리코드(고객 사진, 고객이름, 지역, 생년월일, 담당자, 주소)를 삽입
-- 필요 추가 데이터 : 지역 (화면에서)
...
VALUES ('이영희', '부산 광역시', 'qwer1234', 'null', '590826', '부산광역시 부산진구');
...
-- 고객 상세보기
-- 최종 사용자 (고객이름) => (고객번호, 고객사진, 고객이름, 생년월일, 담당자이름, 주소)
...
-- 관리 기록 테이블에서 특정 고객에 대한 관리 기록 리스트를 날싸 순으로 내림차순 정렬하여 조회
-- 필요 추가 데이터 : 고객 번호 (화면에서)
...
-- 절차상 사용한 용품의 개수 감수 (사용하는 용품이 있을 경우)
