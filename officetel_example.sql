...
CREATE TABLE board (
    ...
    reply TEXT,
    CONSTRAINT write_fk FOREIGN KEY(writer_id) REFERENCES user (id)
);
...
FROM board WHERE title LIKE '%%' ORDER BY receipt_number DESC
LIMIT 0, 10;
...
-- 답글 작성
UPDATE board SET reply = '재미있게 보았어요', status = true
WHERE receipt_number = 1;
