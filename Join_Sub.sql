...
CREATE TABLE employee(
	employe_number INT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(20),
    age INT,
...
-- 부서코드가 A 인 사원에 대해
-- 사번, 이름, 부서명을 조회하시오.
SELECT E.employe_number '사번', E.name '이름' , D.name '부서명'
FROM employee E INNER JOIN department D
ON E.department_code = D.department_code
...
-- 부서명이 '영업부'인 사원에 대해
-- 사번, 이름, 나이를 조회하시오.
SELECT E.employe_number '사번', E.name '이름', E.age '나이'
FROM employee E INNER JOIN department D
ON E.department_code = D.department_code
WHERE D.name = '영업부';
...
SELECT employe_number, name, age
FROM employee
